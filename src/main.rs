...
fn handle_collision(ball: &mut Ball, paddle: &Paddle, side: Position) -> bool {
    ...
    match side {
        Position::Left => match intersection.w > intersection.h {
            true => {
        ...
        Position::Right => match intersection.w > intersection.h {
            true => {
                ball.rect.y -= to_signum.y * intersection.h;
                ball.vel.y = -to_signum.y * ball.vel.y.abs();
            }
    ...
    loop {
        ...
        match game_state {
            ...
            GameState::Playing => {
                ...
                handle_collision(&mut ball, &player1, Position::Left);
                handle_collision(&mut ball, &player2, Position::Right);
            ...
            GameState::Over => {
                draw_text_ex(
                    &String::from(format!("{winner} Wins!")),
                    screen_width() / 2. - 290.,
                    screen_height() / 2.,
                    TextParams {
                        font_size: 70,
